...
this.addSizer_();}
pfRadiusWidget.prototype=new google.maps.MVCObject();pfRadiusWidget.prototype.distance_changed=function(){if($.pfGeolocationDefaults.unit=='mi'){this.set('radius',((this.get('distance')*1000)*1.609344));}else{this.set('radius',this.get('distance')*1000);}};pfRadiusWidget.prototype.addSizer_=function(){var sizer=new google.maps.Marker({draggable:true,title:'Resize',icon:{url:$.pfGeolocationDefaults.icon2,scaledSize:new google.maps.Size(21,21),anchor:{x:10,y:10}}});sizer.bindTo('map',this);sizer.bindTo('position',this,'sizer_position');var me=this;google.maps.event.addListener(sizer,'drag',function(){me.setDistance();});google.maps.event.addListener(sizer,'dragend',function(){me.setNewPoints();$.pfremovebyresults
